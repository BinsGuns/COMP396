...
public class FirebaseManager : MonoBehaviour
{
    ...
    [HideInInspector] public bool _isMultiplayer;
    [HideInInspector] public bool _isHost;
    ...
    private IEnumerator UpdateUsernameDatabase(string _username)
    {
        ...
        else
        {
            ...
            PlayerPrefs.SetInt("FirstTimeLoading", 0);
            PlayerPrefs.Save();
    ...
    private IEnumerator UpdateSavedDataAvailability(string gameStatus)
    {
        ...
        {
            //Game status updated
            PlayerPrefs.SetString("SavedGameAvailable", "false");

            // if (_isHost)
            //    // _multiplayerSo.IsHost = true;
            //  PlayerPrefs.SetInt("isHost",1);
            // else
            //    // _multiplayerSo.IsHost = false;
            //     PlayerPrefs.SetInt("isHost",0);


            if (!_isMultiplayer)
                SceneLoaderManager.instance.LoadTutorialWorld(); // single player//
            else 
            { // multiplayer
                // SceneLoaderManager.instance.LoadMultiplayerGame();
                 _basicSpawner.JoinGame(_isHost);
            }

          //  SceneLoaderManager.instance.LoadMultiplayerGame(); // multi player
    ...
    private IEnumerator LoadUserData()
    {
        ...
        else
        {
            //Data has been retrieved
            DataSnapshot snapshot = DBTask.Result;
            PlayerPrefs.SetString("PlayerName", snapshot.Child("username").Value.ToString());
            ...
            PlayerPrefs.SetString("InventoryStack", snapshot.Child("inventoryStack").Value.ToString());
            PlayerPrefs.SetString("Equipment", snapshot.Child("equipment").Value.ToString());
            PlayerPrefs.SetString("PlayerPosition", snapshot.Child("position").Value.ToString());
            PlayerPrefs.SetString("SavedGameAvailable", snapshot.Child("savedgame").Value.ToString());
            PlayerPrefs.SetString("HasCompanion", snapshot.Child("companion").Value.ToString());
            PlayerPrefs.SetString("SceneIndex", snapshot.Child("sceneIndex").Value.ToString());
            PlayerPrefs.SetInt("FirstTimeLoading", 1);
            PlayerPrefs.SetInt("ResumeFlagKey", 1);
            PlayerPrefs.Save();

            string sceneIndex = PlayerPrefs.GetString("SceneIndex");
